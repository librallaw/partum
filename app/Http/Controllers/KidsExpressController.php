...
class KidsExpressController extends Controller
{
    //

    public function DoAddKidsExpress(Request $request)
    {
        ...
        if(!empty($request->file('image'))){
            ...
            //dd($imagename);
            $destinationPath = "./kidsexpress";

            if($image->move($destinationPath, $imagename)){

                $newTarget['image'] = "/kidsexpress/".$imagename;
        ...
        return Redirect::route("showViewKidsExpress")
            ->with("message","Kids express account successfully")
            ->with("type","success");

    }


    public function creditKidsExpress(Request $request)
    {
        //  dd("I got here");exit;
        ...
        if(empty($module_id)){
            return Redirect::route("showViewKidsExpress")->with("message", "An error212 occurred, please try again")->with
            ("type", 'danger'); exit;

        }

        //check if module exists in the table
        $check_module = Kidsexpress::where("module_id",$module_id)->first();

        if(count($check_module) < 1){
            return Redirect::route("showViewKidsExpress")->with("message", "An error493 occurred, please try again")->with
            ("type", 'danger'); exit;
        }

        if(Auth::user()->unique_id != $check_module->unique_id){
            return Redirect::route("showViewKidsExpress")->with("message", "An error923 occurred, please try again")->with
            ("type", 'danger'); exit;
        }
        ...
        if($err){
            ...
            return Redirect::route("showViewKidsExpress")->with("message", "An error625w32 occurred")
                ->with("type", 'danger');
        }
        ...
        if($paymentDetails->status){
            ...
            if($paymentDetails->data->status =="success" && $paymentDetails->status ==true) {
                ...
                $balance = Kidsexpress::where("module_id",$module_id)->first();
                if(!empty($balance)){
                    ...
                    return Redirect::route("showViewKidsExpress")->with("message", "An error986 occurred")
                        ->with("type", 'danger');


                }

                return Redirect::route("showViewKidsExpress")->with("message", "Your Payment was Successfully")->with("type", 'success');
            ...
            return Redirect::route("showViewKidsExpress")->with("message","An error occured with your transaction, please try again later")->with("type",'danger');

        }

    }

    public function showCreateExpress()
    {

        return view("kidsexpress.create");
    }


    public function showViewKidsExpress()
    {

        $data['kids'] = Kidsexpress::where("unique_id",Auth::user()->unique_id)->get();
        return view("kidsexpress.kidsexpress",$data);
    }
